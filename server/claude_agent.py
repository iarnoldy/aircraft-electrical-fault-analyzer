...
Respond with properly formatted JSON as specified in the system prompt."""
    ...
    def _call_claude_api(self, context: str) -> Dict:
        ...
        Performance Fix (Issue 2-4): In TESTING mode, returns mock response
        to avoid 10-20 second API calls during integration tests.
        """
        # Check if we're in testing mode (for fast integration tests)
        if os.getenv('TESTING') == 'true':
            logger.info("TESTING mode active - returning mock diagnostic response")
            return self._get_mock_diagnostic_response()
    ...
    def _get_mock_diagnostic_response(self) -> Dict:
        """
        Fast mock response for integration testing

        Performance Fix (Issue 2-4): This eliminates 10-20 second API calls
        during automated tests, reducing test time from 2+ minutes to <10 seconds.

        Returns realistic diagnostic response that matches Claude API structure.
        """
        return {
            "diagnosis": "Mock diagnosis for testing - Battery voltage low, possible alternator failure",
    ...
    def _get_fallback_diagnostic(self, symptoms: str, system_state: Dict,
        ...
        # VOLTAGE DROP / LOAD ISSUE DIAGNOSIS
        elif ('voltage drop' in symptoms_lower or 'drops' in symptoms_lower or
              'dim lights' in symptoms_lower or 'flicker' in symptoms_lower):
            diagnostic['diagnosis'] = "Voltage drop under load - likely inadequate alternator output or excessive wiring resistance"
            diagnostic['confidence_level'] = 0.82
            diagnostic['probable_causes'] = [
                {"cause": "Alternator output insufficient for electrical load", "probability": 0.5,
                 "reasoning": "Alternator cannot provide enough current when high-load devices activated"},
                {"cause": "Excessive voltage drop in main power wiring", "probability": 0.3,
                 "reasoning": "Undersized wires or poor connections causing power loss under load"},
                {"cause": "Weak battery unable to supplement alternator", "probability": 0.15,
                 "reasoning": "Battery should provide surge current but may be degraded"},
                {"cause": "Loose or corroded main bus connections", "probability": 0.05,
                 "reasoning": "High-resistance connection at bus bar or master contactor"}
            ]
            diagnostic['troubleshooting_steps'] = [
                {
                    "step": 1,
                    "action": "Measure voltage at battery with engine running, then turn on landing lights",
                    "expected_result": f"Voltage should remain at {specs['alternator_output']}V, drop no more than 0.5V when load applied",
                    "decision_point": "If voltage drops below {specs['alternator_min']}V, alternator output is insufficient",
                    "safety_note": "Keep clear of propeller during engine run"
                },
                {
                    "step": 2,
                    "action": "Measure current draw with clamp meter when symptoms occur",
                    "expected_result": "Total electrical load should not exceed alternator rating (typically 40-60A)",
                    "decision_point": "If load exceeds alternator capacity, reduce electrical loads or upgrade alternator",
                    "safety_note": "Clamp meter around main power wire from alternator"
                },
                {
                    "step": 3,
                    "action": "Perform voltage drop test from alternator output to main bus",
                    "expected_result": "Voltage drop should be less than 0.3V under full load",
                    "decision_point": "If drop exceeds 0.5V, check wire size and all connections for resistance",
                    "safety_note": "Test under actual load conditions, not open circuit"
                },
                {
                    "step": 4,
                    "action": "Check alternator output with carbon pile load tester",
                    "expected_result": "Should maintain rated voltage at rated amperage output",
                    "decision_point": "If output drops below 90% of rating, alternator needs rebuild or replacement",
                    "safety_note": "Alternator and load tester will be very hot during this test"
                },
                {
                    "step": 5,
                    "action": "Inspect main power wire gauge from alternator to bus",
                    "expected_result": "Minimum 6 AWG for 12V system, 4 AWG for 28V system",
                    "decision_point": "If wire gauge is inadequate, upgrade to larger wire per electrical load analysis",
                    "safety_note": "Refer to aircraft electrical system diagram for proper wire routing"
                }
            ]
            diagnostic['recommendations'] = [
                "Verify alternator is producing rated output at normal operating RPM",
                "Upgrade to higher-capacity alternator if electrical loads have been added to aircraft",
                "Install voltmeter if not present to monitor electrical system health",
                "Consider LED lighting upgrade to reduce electrical load (70-80% reduction)",
                "Document all measurements in maintenance log for trend analysis",
                "If alternator output is adequate, check for parasitic loads or failing components"
            ]
        ...
        # CIRCUIT BREAKER TRIP DIAGNOSIS
            ...
            diagnostic['recommendations'] = [
                "Replace faulty circuit breaker if thermal characteristics are degraded",
                "Inspect wiring for chafing, pinch points, or insulation damage",
                "Verify all devices on circuit are within rated specifications",
                "Check for moisture intrusion in avionics bay or electrical panels"
            ]
        ...
        # GENERIC ELECTRICAL ISSUE
            ...
            diagnostic['probable_causes'] = [
                {"cause": "Loose or corroded electrical connection", "probability": 0.4,
                 "reasoning": "Most common cause of intermittent electrical issues"},
                {"cause": "Inadequate alternator output under load", "probability": 0.3,
                 "reasoning": "Voltage drops when high-current loads are activated"},
                {"cause": "Weak battery with low capacity", "probability": 0.2,
                 "reasoning": "Battery cannot maintain voltage under electrical load"},
                {"cause": "Excessive voltage drop in wiring", "probability": 0.1,
                 "reasoning": "Undersized or damaged wiring causing power loss"}
            ]
            diagnostic['troubleshooting_steps'] = [
                {
                    "step": 1,
                    "action": "Perform complete electrical system check starting with battery",
                    "expected_result": f"Battery: {specs['battery_nominal']}V at rest, Alternator: {specs['alternator_output']}V at 1500 RPM",
                    "decision_point": "If battery below {specs['battery_min']}V, charge or replace; If alternator below {specs['alternator_min']}V, diagnose charging system",
                    "safety_note": "Follow proper meter safety procedures, keep clear of propeller"
                },
                {
                    "step": 2,
                    "action": "Measure voltage at battery terminals with all electrical loads turned ON",
                    "expected_result": f"Voltage should remain above {specs['battery_min']}V with all loads active",
                    "decision_point": "If voltage drops excessively, isolate individual loads to find excessive draw",
                    "safety_note": "Monitor ammeter to ensure total load doesn't exceed alternator capacity"
                },
                {
                    "step": 3,
                    "action": "Check all grounds for cleanliness and tightness, especially engine ground strap",
                    "expected_result": "Less than 0.1 ohm resistance between ground point and airframe",
                    "decision_point": "Clean and retighten any connection showing resistance above 0.2 ohms",
                    "safety_note": "Poor grounds can cause intermittent issues and potential fire hazard"
                },
                {
                    "step": 4,
                    "action": "Inspect all electrical connections for corrosion, heat damage, or looseness",
                    "expected_result": "Clean, tight connections with no discoloration or corrosion",
                    "decision_point": "Clean corroded terminals with electrical contact cleaner, replace heat-damaged connectors",
                    "safety_note": "Disconnect battery before cleaning connections"
                },
                {
                    "step": 5,
                    "action": "Perform voltage drop test on main power circuits under load",
                    "expected_result": "Voltage drop less than 0.5V from alternator to bus, less than 0.2V for avionics circuits",
                    "decision_point": "If voltage drop exceeds limits, check wire gauge and connection integrity",
                    "safety_note": "Test with actual load, not just meter"
                }
            ]
            diagnostic['recommendations'] = [
                "Document all voltage measurements in aircraft maintenance log",
                "Clean and apply dielectric grease to all electrical connections",
                "Perform complete electrical load analysis to identify high-current draws",
                "Consider battery load test if symptoms worsen with electrical loads",
                "If issue persists, consult certified A&P mechanic with electrical expertise",
                "Review aircraft electrical system schematic for troubleshooting guidance"
            ]
